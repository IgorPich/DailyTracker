...
$expectedIdentifier = 'com.igorpich.formlog.authoritysmoke'
$smokeRoot = Join-Path $env:APPDATA $expectedIdentifier
$resultPath = Join-Path $smokeRoot 'authority-smoke-result.json'

if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
  throw "Authority smoke executable does not exist: $executable"
}
...
if (Test-Path -LiteralPath $smokeRoot) {
  ...
  Write-Output "Archived prior isolated authority smoke data: $archivePath"
}
...
function Invoke-SmokeRun([string]$Label) {
  ...
  Write-Output "$Label PASS — SQLite $($result.sqliteVersion), schema $($result.schemaVersion), revision $($result.revision)"
}

Invoke-SmokeRun 'Fresh authority WebView smoke'
Invoke-SmokeRun 'Existing authority WebView smoke'
