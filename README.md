...
- zalecany instalator: `apps/desktop/src-tauri/target/release/bundle/nsis/GreekGod_3.0.3_x64-setup.exe`,
- instalatory MSI: `apps/desktop/src-tauri/target/release/bundle/msi/GreekGod_3.0.3_x64_pl-PL.msi` i `GreekGod_3.0.3_x64_en-US.msi`.

Do normalnej instalacji uruchom plik `GreekGod_3.0.3_x64-setup.exe`. Instalator działa dla bieżącego użytkownika, dodaje GreekGod do menu Start oraz skrót na pulpicie i konfiguruje usługę synchronizacji uruchamianą przez Harmonogram zadań. Kliknięcie systemowego `X` kończy aplikację Desktop; usługa synchronizacji działa niezależnie.
...
apps/
└── desktop/
    ├── src/              # React UI, stan, dane, serwisy i narzędzia wersji 3.0.3
