...
// PC-first 4.0 keeps the existing shared/Mobile empty-store seed identities unchanged.
export const DEFAULT_TEMPLATES: TrainingTemplate[] = DEFAULT_TEMPLATE_SEEDS.map((item) => item.mobile)

// Desktop canonical references are explicit seed data, never a history identity resolver.
